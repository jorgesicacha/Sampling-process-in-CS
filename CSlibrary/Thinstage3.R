...
thirdthinning <- function(input){
  ...
  if(dim(class_prob)[1]!=nspecies | dim(class_prob)[2]!=nspecies) stop(paste0("Dimensions of classification probabilities should be nspecies= ", nspecies," x nspecies= ",nspecies)) 
  ...
  for(i in 1:nspecies){
    secondstage$Eco_PPFinal_detect[[i]]$error <- apply(secondstage$Eco_PPFinal_detect[[i]]@data, 1,function(x){which(rmultinom(1,1,p=(class_prob[i,]))==1)})
    secondstage$Eco_PPFinal_detect[[i]]$true_species <- i
  }
  
  #Putting all the data together
  cit_data <- list()
  for(i in 1:nspecies){
    cit_data[[i]] <- do.call("rbind",lapply(secondstage$Eco_PPFinal_detect, function(x) x[which(x@data$error ==i),]))
  }
