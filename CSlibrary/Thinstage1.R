...
{
p_sampling <- length(sampling_idxs)
sampling_form <- c()
...
Samp_PP <- rLGCP(model="matern",mu=eval(parse(text=sampling_linpred)),
                 var=input$sampling$hyperparameters[[1]],scale=input$sampling$hyperparameters[[2]]/sqrt(8),nu=1,win = win,xy=list(x=x0,y=y0))
...
r <- raster(df.sp2)
xres <- sampling_covs.im[[1]]$xstep;yres <- sampling_covs.im[[1]]$ystep## Raster resolution
